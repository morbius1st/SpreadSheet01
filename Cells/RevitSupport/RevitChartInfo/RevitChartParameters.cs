...
namespace SpreadSheet01.RevitSupport.RevitChartInfo
{
	...
	public class RevitChartParameters
	{
		...
		private static void assignParameters()
		{
			...
			// data parameters
			// 0
			pd = new ParamDesc("Name"                  , ChartNameIdx          , 0,    DATA, TEXT, READ_VALUE_REQUIRED, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 1
			pd = new ParamDesc("Description"           , ChartDescIdx          , 0,    DATA, TEXT, READ_VALUE_REQUIRED, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 2
			pd = new ParamDesc("Sequence"              , ChartSeqIdx           , 0,    DATA, TEXT, READ_VALUE_OPTIONAL, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 3
			pd = new ParamDesc("Excel File Path"       , ChartFilePathIdx      , 0,    DATA, FILE_PATH, READ_VALUE_REQUIRED, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 4
			pd = new ParamDesc("Excel WorkSheet Name"  , ChartWorkSheetIdx      , 0,    DATA, TEXT, READ_VALUE_REQUIRED, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 5
			pd = new ParamDesc("Cell Family Name"      , ChartFamilyNameIdx      , 0,    DATA, TEXT, READ_VALUE_REQUIRED, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 6
			pd = new ParamDesc("Update Scheme"         , ChartUpdateTypeIdx      , 0,    DATA, UPDATE_TYPE, READ_VALUE_OPTIONAL, READ_FROM_PARAMETER);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);
			// 7
			pd = new ParamDesc("Cells With Errors"     , ChartHasErrorsIdx      , 0,    MUST_EXIST, IGNORE, READ_VALUE_IGNORE, NOT_USED);
			assignParameter(pd.ParamIndex              , pd.ShortName     , pd);

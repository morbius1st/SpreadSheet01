...
{
	public class VariableDefinitions  : ADefinitionBase<DefVar>
	{
		...
		public override DefVar Invalid => new DefVar("Invalid", null, null, VT_INVALID, PGV_INVALID, -1, -1);
		public override DefVar Default => new DefVar("Default", null, null, VT_DEFAULT, PGV_DEFAULT, 0, 0);
		...
		{
			idDefArray = new DefVar[MAX_TOKENS];
		...
		private static int MakeDefVar(int idx, int seq, string desc, string val, string term, ParseGroupVar pgv)
		{
			...
			idDefArray[idx] = new DefVar(desc, val, term, VT_STRING, pgv, seq, (int) VT_ID_VAR_KEY + seq);
